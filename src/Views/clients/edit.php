<div class="mb-8">
    <h1 class="text-4xl font-black text-secondary tracking-tighter">Editar Cliente</h1>
    <p class="text-gray-500 mt-2 font-medium">Actualiza la información de <?= h($client->name) ?></p>
</div>

<div class="card p-10 shadow-2xl">
    <form method="POST" action="/clients/edit?id=<?= $client->id ?>" x-data="{ 
              name: '<?= h($client->name) ?>', 
              email: '<?= h($client->email ?? '') ?>', 
              phone: '<?= h($client->phone ?? '') ?>', 
              company: '<?= h($client->company ?? '') ?>' 
        ...
        <div class="mb-8">
            ...
            <textarea id="notes" name="notes" class="input-standard h-32"
                placeholder="Información adicional sobre el cliente..."><?= h($client->notes ?? '') ?></textarea>
        </div>
